...
import marytts.language.ar.ArabicConfig;
 ...
 *
 */
public class ArabicConfigTest {
	private static final Locale ARABIC = new Locale("ar");

	@Test
	public void isNotMainConfig() throws MaryConfigurationException {
		MaryConfig m = new ArabicConfig();
		assertFalse(m.isMainConfig());
	}
	
	@Test
	public void canGet() {
		MaryConfig m = MaryConfig.getLanguageConfig(ARABIC);
		assertNotNull(m);
		assertTrue(((LanguageConfig)m).getLocales().contains(ARABIC));
	}
